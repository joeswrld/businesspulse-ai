-- NoteX Dashboard Setup Script
-- Run this in your Supabase SQL Editor to set up the required tables

-- 1. Create AI Jobs table for tracking AI processing status
...
-- 2. Create Sync Logs table for tracking data synchronization status
...
-- 3. Add plan field to profiles table if it doesn't exist
...
-- 9. Enable realtime for the new tables (important for live updates!)
...
INSERT INTO ai_jobs (user_id, job_type, status) 
SELECT auth.uid(), 'sample_job', 'completed' 
WHERE EXISTS (SELECT 1 FROM auth.users WHERE id = auth.uid())
ON CONFLICT DO NOTHING;

-- Success message
SELECT 'NoteX Dashboard setup completed successfully!' as status;