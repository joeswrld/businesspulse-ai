...
CREATE POLICY "Team members can view their teams" ON teams FOR SELECT USING (owner_id = auth.uid() OR EXISTS (SELECT 1 FROM team_members WHERE team_members.team_id = teams.id AND team_members.user_id = auth.uid()));
...
SELECT 'Teams system setup completed successfully!' as status;