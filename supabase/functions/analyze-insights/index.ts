...
serve(async (req)=>{
  ...
  try {
    ...
    // Check usage limits
    const { data: usageData, error: usageError } = await supabase.from('usage_tracking').select('insights_count').eq('user_id', userId).single();
    ...
    const { data: subscriptionData } = await supabase.from('user_subscriptions').select('*').eq('user_id', userId).single();
...
`;
    ...
    // FIXED: Use the correct API endpoint with API key as URL parameter
    ...
    try {
      ...
      if (!analysis.summary || !analysis.key_themes || !analysis.suggested_actions || !analysis.trends || !analysis.performance || !analysis.sentiment) {
