...
CREATE POLICY IF NOT EXISTS "Users can view their own whatsapp links" ON whatsapp_links
  FOR SELECT USING (
    project_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can insert their own whatsapp links" ON whatsapp_links
...
CREATE POLICY IF NOT EXISTS "Users can update their own whatsapp links" ON whatsapp_links
  FOR UPDATE USING (
    project_id IN (
      SELECT id FROM projects WHERE user_id = auth.uid()
    )
...
CREATE POLICY IF NOT EXISTS "Users can delete their own whatsapp links" ON whatsapp_links
  FOR DELETE USING (
    project_id IN (
      SELECT id FROM projects WHERE user_id = auth.uid()
    )
